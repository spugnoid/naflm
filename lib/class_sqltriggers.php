    ...
    protected static function _player($T_SQLTRIG, array $argv) {
        ...
        switch ($T_SQLTRIG) {
            ...
            case T_SQLTRIG_PLAYER_DPROPS: 
                mysql_query("CALL getPlayerDProps($pid, @inj_ma,@inj_av,@inj_ag,@inj_st,@inj_ni, @ma,@av,@ag,@st, @ma_ua,@av_ua,@ag_ua,@st_ua, @value, @status, @date_died, @s_played, @wants_retire)") or die(mysql_error());
                mysql_query("UPDATE players SET 
                            ...
                            value = @value, status = @status, date_died = @date_died, seasons_played = @seasons_played, wants_retire = @wants_retire WHERE player_id = $pid") or die(mysql_error());
