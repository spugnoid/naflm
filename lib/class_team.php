...
class Team
{
    ...
    public function delete()
    {
         ...
         */
        if ($this->isDeletable()) {
            $query = "DELETE FROM match_data WHERE f_team_id = $this->team_id";
            mysql_query($query); // These entries occur only when players are imported.
            $query = "DELETE FROM players WHERE owned_by_team_id = $this->team_id";
            mysql_query($query);
            $query = "DELETE FROM teams WHERE team_id = $this->team_id";
            mysql_query($query);
            $query = "DELETE FROM mv_players WHERE f_tid = $this->team_id";
            mysql_query($query);
            $query = "DELETE FROM mv_teams WHERE f_tid = $this->team_id";
            mysql_query($query);
    ...
    public function drop($thing)
    {
        ...
        if ($this->unbuy($thing)) {
            ...
            # Do not allow a situation, where we have removed the team "thing", and were not able to throw the refund away.
        }
        return false;
    }
