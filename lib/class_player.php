...
class Player
{
    ...
    public function __construct($player_id)
    {
        ...
        if ($this->type == PLAYER_TYPE_JOURNEY) {
            # Check if player is journeyman like this - don't assume setStatusses() has ben called setting $this->is_journeyman.
            $this->position .= ' [J]';
    ...
    public function setChoosableSkills()
    {
        ...
        foreach (array_reverse($IRs) as $IR) {
            list($D1, $D2) = $IR;
            switch ($D1 + $D2) {
                case 12:$chr = array(ST);
                    break;
                case 11:$chr = array(AG);
                    break;
                case 10:$chr = array(MA, AV);
                    break;
                default:$chr = array();
                    break;
            }
    ...
    public function mayHaveNewSkill()
    {
        ...
        foreach (array_reverse($sparray) as $rank => $details) {
            # Loop through $sparray reversed so highest ranks come first.
    ...
    {
        $query = "UPDATE players SET incentive = seasons_played * 20000 WHERE wants_retire = 'Yes' AND player_id = $this->player_id";
        return mysql_query($query);
    }
    ...
    public function getSkillsStr($HTML = false)
    {
        ...
        if ($this->ach_ma > 0) {
            array_push($chrs, "+$this->ach_ma Ma");
        }

        if ($this->ach_st > 0) {
            array_push($chrs, "+$this->ach_st St");
        }

        if ($this->ach_ag > 0) {
            array_push($chrs, "+$this->ach_ag Ag");
        }

        if ($this->ach_av > 0) {
            array_push($chrs, "+$this->ach_av Av");
        }
    ...
    public function getInjsStr($HTML = false)
    {
        ...
        $injs = array();
        if ($this->inj_ma > 0) {
            array_push($injs, "-$this->inj_ma Ma");
        }

        if ($this->inj_st > 0) {
            array_push($injs, "-$this->inj_st St");
        }

        if ($this->inj_ag > 0) {
            array_push($injs, "-$this->inj_ag Ag");
        }

        if ($this->inj_av > 0) {
            array_push($injs, "-$this->inj_av Av");
        }

        if ($HTML) {
            if ($this->inj_ni > 0) {
                array_push($injs, "<font color='red'>$this->inj_ni Ni</font>");
            }

        } else {
            if ($this->inj_ni > 0) {
                array_push($injs, "$this->inj_ni Ni");
            }
